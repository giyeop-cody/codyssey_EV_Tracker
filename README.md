...
- 중복 발화(실측 ~79회/일)는 **수집 간격 하한 게이트**가 흡수한다 — 마지막 수집(meta.generatedAt) 후 30분 미만 run은 codyssey 호출 0건으로 스킵. `COLLECT_MIN_INTERVAL_MIN`로 조정, 수동 지정(`--year/--month/--days`)·`FORCE_DETAIL`·`COLLECT_FORCE=1`은 우회.
