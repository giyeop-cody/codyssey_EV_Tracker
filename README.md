...
codyssey.kr의 동료평가 활동(평가/슬롯 패턴)을 GitHub Actions가 주기적으로 수집해
월별 JSON으로 저장하고, GitHub Pages 정적 대시보드로 시각화합니다.

> 화면에는 **완료·진행 중인 평가만** 표시됩니다. 요청/예약·취소·거절 건은 표시하지 않습니다(데이터는 JSON에 보존).
>
...
- 동료평가 **평가/피평가 횟수**를 월 단위로 수집·비교한다 (취소·거절 데이터도 수집되지만 화면에는 표시하지 않는다).
- **언제 평가가 몰리는지**(시간대×요일) 패턴을 본다.
...
- 평가 상태(`fixedCd`) 해석: `00006`=완료, `00005`/`00004`=취소. 화면에는 완료·진행만 표시한다(2026-08-01 표시 정책).
...
- 중복 발화(실측 ~79회/일)는 **수집 간격 하한 게이트**가 흡수한다 — 마지막 수집(meta.generatedAt) 후 60분 미만 run은 codyssey 호출 0건으로 스킵. `COLLECT_MIN_INTERVAL_MIN`로 조정, 수동 지정(`--year/--month/--days`)·`FORCE_DETAIL`·`COLLECT_FORCE=1`은 우회.
...
- Pages 주소 접속 → 월 선택 → 캘린더/랭킹/히트맵
- 🔍 검색: 이름(**평가한 사람** 기준, 받은 평가는 제외)과 과제명으로 필터 (둘 다 입력 시 AND) → 결과 클릭 시 상세. "전체 월" 체크 시 과거 전체 월 스캔
- 날짜 클릭 → 그날의 평가 목록, 이벤트 클릭 → 평가 상세(점수/결과/시각 등)
...
```
> **(선택) 공유 로스터 허브 연동**: `codyssey_roster_hub`(비공개)를 쓰는 환경이면 Secret `HUB_PAT`(허브 Contents:Read PAT)를 등록하세요. 4시간마다 갱신되는 길드 로스터를 허브에서 받아 길드 API 호출을 줄입니다. 미등록이면 알아서 자체 수집으로 돌아갑니다. Codespace 로그인 서버로 허브 레포의 `CODYSSEY_SESSION`까지 한 번에 갱신하려면 Codespaces 비밀 `GH_SYNC_EXTRA_REPOS=<owner>/codyssey_roster_hub`를 추가하세요.
