...
├── .github/workflows/
│   ├── collect.yml          # 30분 주기 수집 + data/ 커밋
│   └── pages.yml            # 정적 대시보드 배포
├── dashboard/
│   ├── index.html           # 평가 트래커 대시보드 (Pages로 배포, 바닐라 JS)
│   ├── styles.css
│   ├── app.js               # 렌더링 + MOCK 모드(데이터 없으면 가상 데이터)
│   ├── server.js            # Codespace 세션-싱크 서버 (로그인 폼 + Secret 등록)
...
├── docs/API_DISCOVERY.md    # DevTools로 평가 API 찾아 꽂는 방법
└── README.md
```
...
```json
{
  ...
  "members": [ { "mbrId": "sha1:090dc56a", "name": "홍길동", "level": 2, "guild": "3길드" } ],
  "events": [
    {
      "evalId": "630003",
      "slotDateTime": "2026-07-14T09:00:00+09:00",
      "endTime": "09:30",
      "evaluatorId": "sha1:090dc56a", "evaluatorName": "홍길동",
      "evaluateeId": "sha1:61b76ab4", "evaluateeName": "김철수",
